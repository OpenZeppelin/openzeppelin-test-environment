const { accounts, provider, contract, web3, defaultSender } = require('@openzeppelin/test-environment');
...
describe('default config', function() {
  ...
  describe('blockchain', function() {
    ...
    it('fails to deploy a contract over the limit', async function() {
      try {
        const fatty = await Fatty.new();
        expect.fail('Attempted to deploy a contract over the limit');
      } catch {}
    });
  });
});
